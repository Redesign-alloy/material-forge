...
const MONTHLY_CREDIT_LIMIT = 500;
...
export default function Usage() {
  ...
  const totalCreditsUsed = searchCount * CREDITS_PER_ANALYSIS;
  const remainingCredits = Math.max(0, MONTHLY_CREDIT_LIMIT - totalCreditsUsed);
  const usagePercent = Math.min(100, (totalCreditsUsed / MONTHLY_CREDIT_LIMIT) * 100);
  ...
  const stats = [
    ...
    },
    {
      ...
      icon: TrendingUp,
      color: 'success',
      description: `${searchCount * CREDITS_PER_ANALYSIS} credits used this period`,
    },
    {
      ...
      icon: Activity,
      color: 'warning',
      description: `Each analysis costs ${CREDITS_PER_ANALYSIS} credits`,
    },
  ];
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-sm font-mono text-foreground">
              {loading ? '...' : `${totalCreditsUsed} / ${MONTHLY_CREDIT_LIMIT}`}
            </span>
          </div>
          ...
          <div className="flex items-center justify-between mt-2">
            <span className="text-xs text-muted-foreground">
              {loading ? '' : `${searchCount} analyses × ${CREDITS_PER_ANALYSIS} credits each`}
            </span>
            <span className="text-xs text-muted-foreground">
              {loading ? '' : `${remainingCredits} credits remaining`}
            </span>
          </div>
        </div>
          ...
          <div className="relative">
            <svg className="w-48 h-48 transform -rotate-90">
              ...
              <motion.circle
                ...
                animate={{ 
                  strokeDashoffset: 502.65 - (502.65 * usagePercent) / 100 
                }}
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className="text-4xl font-bold text-foreground font-mono">
                {loading ? '...' : `${Math.round(usagePercent)}%`}
              </span>
